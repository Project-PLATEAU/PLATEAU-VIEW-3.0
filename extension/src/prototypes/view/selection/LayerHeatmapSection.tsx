...
export const LayerHeatmapSection: FC<LayerHeatmapSectionProps> = ({ layers }) => {
      ...
      () =>
        atom(get => {
          ...
          return colorSchemes.every(
            (colorScheme): colorScheme is Extract<typeof colorScheme, { type: "quantitative" }> =>
  ...
  const valueRange = useAtomValue(
    useMemo(
      () =>
        atom(get => {
          const ranges = layers
            .map(layer => "valueRangeAtom" in layer && get(layer.valueRangeAtom))
            .filter(isNotFalse);
          const minValue = min(ranges, ([min]) => min);
          const maxValue = max(ranges, ([, max]) => max);
          return minValue != null && maxValue != null ? [minValue, maxValue] : null;
        }),
      [layers],
    ),
  );
