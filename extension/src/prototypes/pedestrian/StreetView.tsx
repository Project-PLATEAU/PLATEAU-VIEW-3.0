...
export const streetViewVisibleAtom = atom(get => get(streetViewVisiblePrimitiveAtom));
...
export interface StreetViewProps
  ...
  zoom?: number;
  radius?: number;
  onLoad?: (pano: string, location: Location, headingPitch: HeadingPitch, zoom: number) => void;
  ...
  (
    ...
    usePanoramaLocationChange(panorama, location => {
      ...
      if (!locationChangedRef.current) {
        ...
        onLoad?.(panorama.getPano(), location, panorama.getPov(), panorama.getZoom());
    ...
    return <Root ref={mergeRefs([ref, forwardedRef])} {...props} hidden={hasError} />;
  },
);
