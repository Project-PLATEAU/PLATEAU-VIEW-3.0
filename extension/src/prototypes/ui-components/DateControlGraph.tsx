...
const RiseSetGradient: FC<{
  ...
  const id = useId();
  const rect = useMemo(() => createRoundedRectPath({ width, height, radius: 5 }), [width, height]);
...
> = ({ width, height, borderRadius = 5, dateAtom, observerAtom, solsticesAtom, riseSetAtom }) => {
  const date = useAtomValue(dateAtom);
  const observer = useAtomValue(observerAtom);
  const { summer: summerSolstice, winter: winterSolstice } = useAtomValue(solsticesAtom);
...
export const DateControlGraph: FC<DateControlGraphProps> = props => {
  ...
  return (
    <Root ref={ref} {...props}>
      {width != null && height != null && <Graph {...props} width={width} height={height} />}
    </Root>
  );
};
