...
import { createMeshData, type MeshData, type MeshDataInput } from "./createMeshData";
...
export interface WorkerParseCSVResult extends Omit<ParseCSVResult, "codes" | "values"> {
  codes: ArrayBuffer;
  values: ArrayBuffer;
}

export interface WorkerCreateMeshDataParams extends Omit<MeshDataInput, "codes" | "values"> {
  codes: ArrayBuffer;
  values: ArrayBuffer;
}
...
expose({
  parseCSV: ({ data, options }: WorkerParseCSVParams): TransferDescriptor<WorkerParseCSVResult> => {
