...
export function composeIdentifier({ type, subtype, key, index }: IdentifierParams): string {
...
export function parseIdentifier(identifier: string): ParsedIdentifierParams {
  ...
  const [type, subtype, key, index] = components.map(value => (value !== "" ? value : undefined));
