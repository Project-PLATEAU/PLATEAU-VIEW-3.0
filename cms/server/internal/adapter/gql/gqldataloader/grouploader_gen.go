...
type GroupLoader struct {
	...
	cache map[gqlmodel.ID]*gqlmodel.Group
...
func (l *GroupLoader) Load(key gqlmodel.ID) (*gqlmodel.Group, error) {
...
func (l *GroupLoader) LoadThunk(key gqlmodel.ID) func() (*gqlmodel.Group, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*gqlmodel.Group, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *GroupLoader) LoadAll(keys []gqlmodel.ID) ([]*gqlmodel.Group, []error) {
	results := make([]func() (*gqlmodel.Group, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	groups := make([]*gqlmodel.Group, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *GroupLoader) LoadAllThunk(keys []gqlmodel.ID) func() ([]*gqlmodel.Group, []error) {
	results := make([]func() (*gqlmodel.Group, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *GroupLoader) Prime(key gqlmodel.ID, value *gqlmodel.Group) bool {
	l.mu.Lock()
	var found bool
...
func (l *GroupLoader) Clear(key gqlmodel.ID) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *GroupLoader) unsafeSet(key gqlmodel.ID, value *gqlmodel.Group) {
	if l.cache == nil {
		l.cache = map[gqlmodel.ID]*gqlmodel.Group{}
	}
	l.cache[key] = value
}
...
func (b *groupLoaderBatch) keyIndex(l *GroupLoader, key gqlmodel.ID) int {
