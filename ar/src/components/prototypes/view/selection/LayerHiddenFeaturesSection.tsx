...
import { InspectorItem, MinusIcon, ParameterList, ValueParameterItem } from "../../ui-components";

function hasHiddenFeaturesAtom(
  values: readonly LayerModel[],
): values is ReadonlyArray<Extract<LayerModel, { hiddenFeaturesAtom: unknown }>> {
...
export const LayerHiddenFeaturesSection: FC<LayerHiddenFeaturesSectionProps> = ({ layers }) => {
  ...
  const [hiddenFeatureCount, showAllHiddenFeatures] = useAtom(hiddenFeaturesAtom);
      ...
      <InspectorItem>
        <ParameterList>
          <ValueParameterItem
            label="非表示の建築物"
            value={
              <Stack direction="row" spacing={1} alignItems="center" sx={{ marginRight: -1 }}>
                <Typography variant="body2" color="text.secondary">
                  {hiddenFeatureCount}個
                </Typography>
                <Tooltip title="再表示">
                  <IconButton aria-label="再表示" onClick={showAllHiddenFeatures}>
