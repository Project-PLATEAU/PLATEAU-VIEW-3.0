...
import { differenceBy, intersectionBy, without } from "lodash-es";
...
export function atomsWithSelection<T>({
  ...
  const selectionAtom = atom(
    ...
    (_, set, action: SetStateAction<readonly T[]>) => {
      set(selectionPrimitiveAtom, prevSelection => {
        const values = typeof action === "function" ? action(prevSelection) : action;
        if (values.length === 0) {
  ...
  const addAtom = atom(null, (_, set, values: readonly T[]) => {
    set(selectionPrimitiveAtom, prevSelection => {
      if (values.length === 0) {
        return prevSelection;
      }
  ...
  const removeAtom = atom(null, (_, set, values: readonly T[]) => {
    set(selectionPrimitiveAtom, prevSelection => {
      if (values.length === 0) {
        return prevSelection;
      }
  ...
  const clearAtom = atom(null, (_, set) => {
    set(selectionPrimitiveAtom, prevSelection => {
      if (prevSelection.length === 0) {
        return prevSelection;
      }
