...
export type DateSliderProps = SetRequired<SliderProps, "min" | "max">;
  ...
  ({ min, max, ...props }, ref) => {
    ...
    const marks = useMemo(() => {
      ...
      const widths = ticks.map(
        (tick, index) => `${(+(ticks[index + 1] ?? max) - +tick) * percentFactor}%`,
      );
      return ticks.map((value, index) => ({
        value: +value,
        label: <Label style={{ width: widths[index] }}>{format(value, "MMM")}</Label>,
      }));
    }, [min, max]);
