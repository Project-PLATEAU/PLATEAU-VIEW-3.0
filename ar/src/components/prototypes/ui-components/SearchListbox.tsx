...
const OuterElement = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>((props, ref) => {
...
export const SearchListbox = forwardRef<HTMLDivElement, SearchListboxProps>(
  ({ itemSize: itemSizeProp, maxItemCount = 10, maxHeight, children, ...props }, forwardedRef) => {
    const theme = useTheme();
    ...
    const getChildSize = useCallback(() => itemSize, [itemSize]);

    const getItemSize = useCallback(() => getChildSize(), [getChildSize]);

    const height = useMemo(() => {
      const intrinsicHeight = sum(itemData.map(getChildSize)) + listPadding * 2;
      if (maxHeight != null) {
        return Math.min(intrinsicHeight, maxHeight - parseFloat(theme.spacing(6)));
      }
      return itemCount < maxItemCount ? intrinsicHeight : maxItemCount * itemSize + listPadding * 2;
    }, [maxItemCount, maxHeight, theme, itemSize, itemData, itemCount, listPadding, getChildSize]);
