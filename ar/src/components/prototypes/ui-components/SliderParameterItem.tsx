...
interface InternalSliderParameterItemProps
  ...
  range?: boolean;
  atom: PrimitiveAtom<number | number[]> | Array<PrimitiveAtom<number | number[]>>;
}

const MIXED = "MIXED";

export const SliderParameterItem = forwardRef<HTMLDivElement, InternalSliderParameterItemProps>(
  (
    {
      label,
      labelFontSize,
    ...
    const handleChange = useCallback(
      (event: Event, value: number | number[], activeThumb: number) => {
        ...
        if (logarithmic) {
          const scaledValue = logarithmic ? inversePseudoLog(value, logarithmicBase) : value;
          const stepValue = Array.isArray(scaledValue)
            ? (scaledValue.map(value => Math.round(value / step) * step) as [number, number])
