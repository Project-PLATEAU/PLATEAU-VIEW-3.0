...
import { importCesium } from "@prototypes/nest-cesium";
import { TileCacheService, type RenderTileOptions } from "@prototypes/nest-tile-cache";
...
function readValue(bytes: Uint8ClampedArray, byteOffset: number): number {
  return (bytes[byteOffset] << 16) | (bytes[byteOffset + 1] << 8) | bytes[byteOffset + 2];
}

function writeValue(value: number, bytes: Uint8ClampedArray, byteOffset: number): void {
...
function writePackedValue(value: number, bytes: Uint8ClampedArray, byteOffset: number): void {
  const rgb =
    value > 0x800000 ? value - 0x800001 : value !== 0x800000 ? 0x7ffffe + value : 0x7fffff;
...
async function upscalePackedBytes(bytes: Uint8ClampedArray): Promise<Uint32Array> {
  const packedValues = new Uint32Array(256 * 256);
  for (let byteOffset = 0, index = 0; byteOffset < bytes.length; byteOffset += 3, ++index) {
...
export class TerrainTileService {
  ...
  private async requestBuffers({ x, y, level }: Coordinates): Promise<Buffer[]> {
  ...
  private async requestTile(coords: Coordinates): Promise<Buffer | undefined> {
    ...
    const byteArrays = buffers.map(buffer => new Uint8ClampedArray(buffer.buffer));
    ...
    for (let byteOffset = 0; byteOffset < this.byteLength; byteOffset += 3) {
      ...
      if (value === 0x800000 && coords.level === 15) {
        if (parentCache == null) {
          ...
          parentCache = {
            parentPackedValues:
              parentPackedBytes != null ? await upscalePackedBytes(parentPackedBytes) : false,
            parentRect,
            rect,
          };
        }
        ...
        if (parentPackedValues !== false) {
          const parentValue = readParentValue(parentPackedValues, parentRect, rect, byteOffset);
